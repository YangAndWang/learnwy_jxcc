...
public class RolePowerDB {
    public static List<RolePower> getRolePowers() {
        ...
        String sql = "select distinct role_power_id ,power_id ,role_id from role_power";
    ...
    public static RolePower getRolePowerById(long id) {
        String sql = "select distinct role_power_id ,power_id ,role_id  from role_power where role_power_id = " + id;
        ResultSet rs = MySQL.excuteSQL(sql);
        RolePower role_power = null;
        try {
            if (rs.next()) {
    ...
    public static int delRolePower(long role_id, long power_id) {
        String sql = "delete from role_power where role_id= " + role_id + " and power_id = " + power_id;
        return MySQL.updateSQL(sql);

    }

    public static int addRolePower(long power_id, long role_id) {
        String sql = "insert into role_power(power_id ,role_id )values( " + power_id + "," + role_id + ")";
        return MySQL.updateSQL(sql);
    }
}
