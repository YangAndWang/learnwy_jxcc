...
public class SysMenuPowerDB {
    public static List<SysMenuPower> getSysMenuPowers() {
        ...
        String sql = "select distinct sys_menu_power_id ,sys_menu_id ,power_id from sys_menu_power";
    ...
    public static SysMenuPower getSysMenuPowerById(long id) {
        String sql = "select distinct sys_menu_power_id ,sys_menu_id ,power_id  from sys_menu_power where sys_menu_power_id = "
                + id;
        ResultSet rs = MySQL.excuteSQL(sql);
        SysMenuPower sys_menu_power = null;
        try {
    ...
    public static int delSysMenuPower(long power_id, long sys_menu_id) {
        String sql = "delete from sys_menu_power where power_id = " + power_id + "  and sys_menu_id = " + sys_menu_id;
        return MySQL.updateSQL(sql);
    }

    public static int addSysMenuPower(long power_id, long sys_menu_id) {
        String sql = "insert into `sys_menu_power`(`power_id`,`sys_menu_id`)values(" + power_id + "," +
                sys_menu_id + ")";
        return MySQL.updateSQL(sql);
    }
}
